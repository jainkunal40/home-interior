...
export function ProjectDetailView({ project, allVendors, allContractors }: { project: any; allVendors?: any[]; allContractors?: any[] }) {
  ...
  const clientPaidExpenses = project.expenseTransactions
    .filter((t: any) => t.paidByClient)
    .reduce((s: number, t: any) => s + t.amount + (t.taxAmount || 0), 0)
  const totalLabor = project.laborEntries.reduce((s: number, t: any) => s + t.totalAmount, 0)
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="pb-20 sm:pb-4">
        {activeTab === 'overview' && <OverviewTab project={project} totalIncome={totalIncome} totalExpenses={totalExpenses} totalLabor={totalLabor} netProfit={netProfit} clientPaidExpenses={clientPaidExpenses} allVendors={allVendors} allContractors={allContractors} />}
        ...
        {activeTab === 'reports' && <ReportsTab project={project} totalIncome={totalIncome} totalExpenses={totalExpenses} totalLabor={totalLabor} netProfit={netProfit} clientPaidExpenses={clientPaidExpenses} />}
      </div>
    </div>
  )
}
